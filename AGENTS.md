...
- **PC-A**: WebHID chat page (`web/chat-usb.html`) sends messages and attachments over USB
- **Flipper Zero**: FAP bridges USB HID ↔ BLE Serial transparently
- **PC-B**: Web Bluetooth chat page (`web/chat-ble.html`) receives and reassembles
...
│   ├── firmware-guide.md   # Flipper firmware build/deploy guide
│   └── attic/
...
The firmware modifications live in a **separate repo**:
`~/projects/flipperzero-firmware/`
...
1. **Web changes**: Edit `web/*.html`, serve with `python3 -m http.server`
2. **Firmware changes**: Edit in `flipperzero-firmware/`, build with `./fbt`
...
- The Deploy flow is BadUSB-shaped by design (HID is the only USB class that survives HID-only policies) and requires physical possession plus explicit on-device action. The typed payload is a fixed, reviewable ASCII artifact (`web/bootstrap.js`).
...
- Flipper never stores full file (single-chunk buffer only)
