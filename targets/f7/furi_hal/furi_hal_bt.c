...
void furi_hal_bt_reinit(void) {
    ...
    /* Service callbacks may point into an external app. Join their dispatcher
     * before freeing profile state or unloading its code. Command responses
     * are delivered by the transport IRQ, independently of this worker. */
    ble_event_thread_stop();
    ...
    // Magic happens here
    if(ble_glue_is_radio_stack_ready()) hci_reset();
    ...
    if(ble_glue_is_alive()) furi_check(ble_glue_reinit_c2());
    ble_glue_stop();
