...
When debugging that symptom, check for duplicates first:
...
`tools/flipper_alive.py` (in the flipper-hid repo) reports ALIVE / HUNG /
...
## 14. Disable persistent bonding for the BLE keyboard profile

macOS can keep a bonded HIDS keyboard connection open, suppressing advertising
and preventing Web Bluetooth discovery. Keep numeric-comparison pairing and
authenticated GATT characteristics, but set `.bonding_mode = false` so the
host does not retain the keyboard bond after the session.
