...
- This local custom firmware and apps repository is
  `/Users/asutov/projects/flipperzero-firmware`, normally on the
  `pocket-airbridge` branch. Pocket AirBridge product assets are in `airbridge/`,
  and its canonical FAP source is `applications_user/pocket_airbridge/`.
...
- The browser-facing BLE transport uses the **custom AirBridge serial UUID
  family** generated in `airbridge/web/airbridge-identity.js`. The required firmware
...
- Firmware, FAP, and web sources are in-tree. There is no second checkout, patch
  bundle, or patch-application workflow. The operational guide is
...
| Path | Purpose |
|------|---------|
| `/Users/asutov/projects/flipperzero-firmware/` | Firmware source tree |
| `airbridge/` | Pocket AirBridge web assets, docs, scripts, and tools |
| `applications_user/pocket_airbridge/` | Canonical Pocket AirBridge FAP source |
