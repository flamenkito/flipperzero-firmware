...
1. Launch a persistent Chrome ONCE, before the first browser step, with a CDP
   ...
   ```bash
   mkdir -p "$HOME/.cache/airbridge-playwright-profile"
   "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" \
     --remote-debugging-port=9222 \
     --user-data-dir="$HOME/.cache/airbridge-playwright-profile" \
...
window showing `http://localhost:8080/chat-usb.html`.”

## Preflight

Confirm without modifying state:

- Main project: `~/projects/flipper-hid`
- Firmware: `~/projects/flipperzero-firmware`
- Firmware branch: `pocket-airbridge`
- Firmware remote: `git@github.com:flamenkito/flipperzero-firmware.git`
...
Use `.agents/skills/flipper-zero-dev/scripts/check-flipper.sh` or
`tools/flipper_alive.py` to distinguish ALIVE, HUNG, and ABSENT states. Port
present but no CLI prompt means HUNG and requires a physical reset.
...
From the firmware checkout, upload and verify:

| Source | Destination |
|---|---|
| `.extapps/pocket_airbridge.fap` | `/ext/apps/USB/pocket_airbridge.fap` |
| `web/bootstrap.js` | `/ext/apps_data/pocket_airbridge/bootstrap.js` |
| `dist/app-usb.html` | `/ext/apps_data/pocket_airbridge/app-usb.html` |
...
Then serve `web/` on localhost, navigate the Playwright browser to
`chat-usb.html`, and use the physical gate protocol for the WebHID chooser.
...
Navigate the intended Playwright window to `chat-ble.html` before asking the
...
UUID family from `web/airbridge-identity.js`.
