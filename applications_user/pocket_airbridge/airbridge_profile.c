...
static FuriHalBleProfileBase* ble_profile_airbridge_start(FuriHalBleProfileParams profile_params) {
    ...
    // GATT budget: battery(8) + AirBridge DIS(9) + serial(12) = 29 + HIDS(~20) = ~49 <= 68.
...
bool airbridge_profile_kb_report(FuriHalBleProfileBase* profile, uint8_t* data, uint16_t len) {
    if(!profile || profile->config != ble_profile_airbridge) return false;
    // The HID map uses IDs 1..3; the HID service API selects its report array by index 0..2.
    // ble_svc_hid_update_input_report uses the gatt-layer convention (true = error);
    // invert here so callers get true = sent, matching ble_svc_airbridge_serial_update_tx.
    return !ble_svc_hid_update_input_report(
        ((BleProfileAirbridge*)profile)->hid_svc, AirbridgeHidReportIdKeyboard - 1, data, len);
}

bool airbridge_profile_kb_release(FuriHalBleProfileBase* profile) {
    uint8_t release_all[8] = {0};
    return airbridge_profile_kb_report(profile, release_all, sizeof(release_all));
}
...
static void
     ...
     * (Restored verbatim from the proven-working era; a keyboard appearance
     * makes macOS demand keyboard-style PIN pairing, which this device cannot
     * perform outside the Deploy typing window.) */
