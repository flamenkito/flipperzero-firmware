...
pub async fn open(link: &mut Link, id: u64, cfg: &Config) -> Result<()> {
    ...
    loop {
        tokio::select! {
            ...
            _ = retry.tick() => link.send(Frame::control(Kind::Open, id, 0)).await?,
            frame = link.receive() => {
                let frame = frame?;
                if frame.session != id { continue; }
                match frame.kind {
                    Kind::Accept => return Ok(()),
    ...
    S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
{
    ...
    let (write_tx, mut write_rx) = mpsc::channel::<Frame>(WINDOW);
    let (done_tx, mut done_rx) = mpsc::channel::<Result<u32>>(WINDOW);
    ...
    if role == Role::Responder {
        emit(link, &mut stats, Frame::control(Kind::Accept, id, 0)).await?;
    }
    loop {
        ...
        tokio::select! {
            result = reader.read(&mut input), if !local_eof && pending.len() < WINDOW => {
                ...
                match frame.kind {
                    ...
                    Kind::Data | Kind::Fin => {
                        if frame.sequence < admitted {
                            ...
                            ensure!(admitted - stats.receive_next < WINDOW as u32, "receive window exceeded");
                            ...
                            if history.len() > WINDOW { history.pop_front(); }
                            continue;
                        }
            ...
            _ = tick.tick() => {
                ...
                if finished.is_none() && last_ping.elapsed() >= cfg.heartbeat {
